...
- **Phase 8 (Systematic Hyperparameter Sweeping & Architecture Tuning):** **READY FOR FULL SWEEP.** The prior six-run result evaluated one fixed configuration per architecture and overstated the processed token count, so it remains invalid. The corrected protocol evaluates three preregistered candidates per architecture across three seeds (18 runs) and rounds full batches upward to meet the 600M-token minimum. See the [phase protocol](phases/phase8.md) and [candidate matrix](phases/phase8_candidates.json).
...
- [**Phase 8: Systematic Hyperparameter Sweeping & Architecture Tuning**](phases/phase8.md) — **READY FOR FULL SWEEP**; the corrected protocol is 18 runs (2 architectures × 3 candidates × 3 seeds), each processing at least 600M tokens.
