...
- **Phase 7 (Full Architecture Assembly & Pilot Pretraining):** [PASS](results/phase7/PASS.md). Both 15M architectures completed 100,000 WikiText-103 steps on 16 TPU v4 chips. Algebraic/standard validation perplexity was 26.919/25.226 (ratio 1.0671), throughput was 2.340M/2.477M tokens/s (ratio 0.9448), and the algebraic arm recorded zero non-finite updates or loss spikes ([hardware evidence](results/phase7/tpu/metrics.json)).
- **Phase 8 (Systematic Hyperparameter Sweeping & Architecture Tuning):** [PASS](results/phase8/metrics.json). Completed all 18 preregistered runs across Seeds 42, 43, 44 on 16 TPU v4 chips (10,815,012,864 tokens total). Fair evaluation without warmup tripping or RMSNorm decay selected `base_high_lr` (Val Loss: 4.0375, PPL: 56.71, std: 0.68%) and `alg_high_lr` (Val Loss: 3.8563, PPL: 47.29, std: 0.17%), establishing a 16.60% lower validation perplexity (ratio 0.8340) for the Algebraic Transformer with zero loss spikes, bounded gradients, and frozen configurations ready for Phase 9.
...
- [**Phase 6: Hardware-Fused Kernels & Algebraic FlashAttention on 16 TPU v4 Pod**](phases/phase6.md) — **VERIFIED PASS** ([PASS.md](results/phase6/PASS.md)) (real JAX Pallas TPU kernel, bounded additive tile accumulation, $98.41\%$–$99.67\%$ Pallas-baseline throughput, lock-free Ring Attention over ICI)
- [**Phase 7: Full Architecture Assembly & Pilot Pretraining**](phases/phase7.md) — **VERIFIED PASS** ([PASS.md](results/phase7/PASS.md)); both architectures completed the full 100,000-step contract on 16 TPU v4 chips.
- [**Phase 8: Systematic Hyperparameter Sweeping & Architecture Tuning**](phases/phase8.md) — **VERIFIED PASS** ([metrics.json](results/phase8/metrics.json)); evaluated 18 runs (2 architectures × 3 candidates × 3 seeds) across 10.82B tokens on 16 TPU v4s; selected `alg_high_lr` (PPL 47.29) and `base_high_lr` (PPL 56.71, ratio 0.8340).
...
5. **Head-to-Head Pilot Pretraining on WikiText-103 (Phase 7):**
   ...
   - **Full execution:** Both architectures completed $100,000$ steps in the same token order. Validation perplexity was $\mathbf{26.919}$ algebraic versus $\mathbf{25.226}$ standard, a passing ratio of $\mathbf{1.0671}$.
   - **Sustained throughput:** Algebraic throughput was $\mathbf{2,340,062\text{ tok/s}}$ versus $\mathbf{2,476,769\text{ tok/s}}$, a passing ratio of $\mathbf{0.9448}$.
   - **Numerical Stability:** Exact zero NaNs, zero Infs, zero loss spikes ($\Delta \mathcal{L} > 1.5$), and peak algebraic gradient norm bounded at $\mathbf{1.000001}$ under BF16 mixed-precision training.
   ...
   - **Historical record:** The superseded 200-step diagnostic remains available in repository history but is not current evidence.
...
6. **Systematic Hyperparameter Sweeping & 125M Calibration on FineWeb-Edu (Phase 8):**
   ...
   - **Evaluation Volume:** 18 complete runs (2 architectures $\times$ 3 candidates $\times$ 3 seeds: 42, 43, 44 on 600.8M tokens each = **10.82 Billion tokens evaluated**).
   - **Empirical Swept Perplexity:** The fair multi-seed evaluation confirmed optimal configurations: `alg_high_lr` achieved mean validation perplexity $\mathbf{47.29}$ (Val Loss: $3.8563$) vs $\mathbf{56.71}$ (Val Loss: $4.0375$) for `base_high_lr` (perplexity ratio $\mathbf{0.8340}$, a $16.60\%$ advantage for the Pure Algebraic Transformer).
   - **Multi-Seed Stability:** Algebraic multi-seed standard deviation was **$0.166\%$** (vs $0.683\%$ for baseline, both well below the $2.0\%$ gate).
   - **Numerical Safety & Zero Transcendental Guarantee:** Exact zero NaNs, zero Infs, zero steady-state loss spikes across all 18 runs, peak gradient norm $\mathbf{1.000}$, and 100% verified AST zero-transcendental purity.
   - **Frozen Winner Configurations:** `results/phase8/algebraic_optimal.json` and `results/phase8/baseline_optimal.json` are verified and frozen to directly govern Phase 9 (125M on 2.5B tokens).
