# Phase 8: Frontier Pretraining: 125M Parameters on 2.5B FineWeb-Edu Tokens
...
- **Paired Seeds:** 3 identical random seeds (Seed 42, Seed 1337, Seed 2026), yielding $2 \text{ architectures} \times 3 \text{ seeds} = \mathbf{6\text{ complete pretraining runs}}$ of 2.5B tokens each.
...
1. **`scripts/run_pretrain_125m.py`**:
   ...
   - Support for CLI flags: `--architecture {algebraic, standard} --seed {42, 1337, 2026} --total_tokens 2500000000`.
...
2. **Forward Dependency Cascading:**
   - Any architectural calibration or optimizer refinement validated during Phase 8 must be forwarded into **Phase 9 (Paper Finalization & Clean-Room Replication)**.
...
- [ ] All 6 pretraining runs (2 architectures $\times$ 3 seeds) complete the full 2.5B token budget with zero unhandled NaNs or divergent loss spikes.
