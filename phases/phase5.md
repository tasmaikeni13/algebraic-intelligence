...
The production API also supports exact Optax denominator semantics
$\hat m/(\sqrt{\hat v}+\epsilon)$ without emitting a raw `sqrt` opcode: for
$\hat v>0$, compute $\sqrt{\hat v}=\hat v\operatorname{rsqrt}(\hat v)$, with an
explicit zero branch. Both modes therefore use the permitted `rsqrt` radical.
...
2. **`tests/test_optimizer.py`**:
   ...
   - JAXPR/HLO audit requiring zero raw `sqrt` instructions and at least one `rsqrt` in every ARDS optimizer step.
...
| **Radical Opcode Audit** | Inspect the compiled ARDS optimizer HLO | Exactly 0 raw `sqrt`; `rsqrt` is present for optimizer preconditioning and scheduling. |
...
- [ ] Compiled ARDS optimizer contains `rsqrt` and no raw `sqrt` opcode.
