...
3. **Algebraic Softmax (A-Softmax):** $\mathbf{S}_n(\mathbf{s})_i = \rho(\hat{s}_i)^n / (\sum_j \rho(\hat{s}_j)^n + \Omega)$ with globally 2-Lipschitz kernel $\rho(x) = x + \sqrt{x^2 + 1}$, sharpening exponent $n = 8 = 2^3$ evaluated via 3 hardware squarings, normalized-coordinate entrywise Jacobian bound $\le n/4 = 2.0$, routing contrast $> 10^5$, and rational attention sink $\Omega \ge 0$.
4. **Octo-Algebraic Cross-Entropy (OACE / $\mathcal{L}_{1/8}$):** Strictly proper non-local power score $8\sum_i y_i p_i^{-1/8}+\frac87\sum_i p_i^{7/8}-\frac{64}{7}\sum_i y_i^{7/8}$, evaluated through a three-`rsqrt` prediction path. Its probability gradient is singular at zero; the verified stability contract is the finite AVN + A-Softmax composed gradient.
5. **Algebraic Divergence (AD):** Strictly proper Pearson $\chi^2$ divergence $D_A(\mathbf{y} \| \mathbf{p}) = \sum y_i^2 / p_i - 1$ and Riemannian Fisher information metric equivalence $\nabla^2 D_A|_{\mathbf{p}=\mathbf{y}} = 2 \nabla^2 D_{\text{KL}}|_{\mathbf{p}=\mathbf{y}}$.
...
| [**`phase2.md`**](phase2.md) | **Octic Algebraic Attention & Entrywise Jacobian Bounds** | A-Softmax 3-stage squaring ($\kappa_8$), normalized-coordinate entrywise Jacobian bound $\le n/4 = 2.0$, dynamic contrast $> 10^5$, and a $\ge100\times$ result in the specified canonical outlier benchmark (not a uniform quantization guarantee). |
...
| [**`phase4.md`**](phase4.md) | **Algebraic Loss Functionals & Information Metrics** | Strictly proper non-local OACE via 3 $\operatorname{rsqrt}$ calls, exact probability gradient and finite composed gradient, Pearson $\chi^2$ divergence, and Fisher information equivalence $2 \nabla^2 D_{\text{KL}}$. |
...
| [**`phase8.md`**](phase8.md) | **Systematic Hyperparameter Sweeping & Architecture Tuning** | Preregistered equal-budget tuning study: 3 candidates per architecture across 3 seeds on at least 600M FineWeb-Edu tokens each (18 runs total) on 16 TPU v4 Pod / v4-32. |
...
| **Phase 2** | **A-Softmax** | Softmax | Attention score distribution and the declared canonical outlier-noise benchmark | Throughput $\ge 90\%$ of Softmax; attention distribution within $\le 5\%$ Wasserstein-1 delta; $\ge100\times$ lower noise only in the specified benchmark, with counterexamples retained. |
...
| **Phase 4** | **OACE** | Cross-Entropy ($-\ln p$) | Classification convergence, strict propriety, and same-domain label-noise characterization | Final accuracy within 5 percentage points; all logit gradients finite; analytical OACE gradient agrees with autodiff. Variance is descriptive, not a superiority gate. |
...
| **Phase 8** | **Sweep** | Standard Transformer | Three preregistered candidates per architecture $\times$ Seeds 42, 43, 44 on at least 600M FineWeb-Edu tokens each (18 runs total) | Fair apples-to-apples hyperparameter discovery for both architectures prior to 2.5B-token main run. |
| **Phase 9** | **Full (125M)** | Standard Transformer (125M) | 2.5B FineWeb-Edu tokens across Seeds 42, 43, 44 | Validation perplexity within $\le 1.08\times$ ($\le 8\%$ degradation); downstream zero-shot means no more than 2 percentage points below baseline. |
...
│   ├── run_hparam_sweep.py          # Phase 8: 18-run preregistered multi-seed sweep
