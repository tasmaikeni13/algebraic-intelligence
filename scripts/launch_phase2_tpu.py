#!/usr/bin/env python3
"""Copy a committed snapshot and run Phase 2 on the existing, idle TPU slice.
...
def main():
    ...
    parser.add_argument("--output",type=Path,default=ROOT/"results/phase2/tpu")
    ...
    subprocess.run(["git","diff","--exit-code","HEAD","--","src","scripts","tests","formal","phases/phase2.md","requirements.txt","requirements-tpu.txt"],cwd=ROOT,check=True)
    ...
    if untracked:
        raise RuntimeError("Commit Phase 2 source files before launching a distributed snapshot.")
    commit=subprocess.check_output(["git","rev-parse","HEAD"],cwd=ROOT,text=True).strip()
    branch=subprocess.check_output(["git","branch","--show-current"],cwd=ROOT,text=True).strip()
    if not branch:
        raise RuntimeError("Launch from a named branch.")
    stamp=datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    label=f"algebraic-phase2-{commit[:12]}-{stamp}"
    remote=f"/tmp/{label}"
    quote=shlex.quote
    with tempfile.TemporaryDirectory(prefix="algebraic-phase2-") as temporary:
    ...
    try:
        ssh(f"cd {quote(remote)} && PYTHONUNBUFFERED=1 venv/bin/python scripts/run_phase2_tpu.py --output {quote(remote+'/measurements')}", "run.log")
    finally:
