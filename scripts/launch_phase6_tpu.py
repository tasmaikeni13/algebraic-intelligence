#!/usr/bin/env python3
"""Copy a committed snapshot and run Phase 6 on the existing, idle TPU slice.
...
def main():
    ...
    subprocess.run(["git", "diff", "--exit-code", "HEAD", "--", "src", "scripts", "tests", "formal", "phases/phase6.md", "requirements.txt", "requirements-tpu.txt"], cwd=ROOT, check=True)
    ...
    if untracked:
        raise RuntimeError(f"Commit Phase 6 source files before launching a distributed snapshot:\n{untracked}")
    ...
    label = f"algebraic-phase6-{commit[:12]}-{stamp}"
    remote = f"/tmp/{label}"
    quote = shlex.quote

    with tempfile.TemporaryDirectory(prefix="algebraic-phase6-") as temporary:
    ...
    try:
        ssh(f"cd {quote(remote)} && PYTHONUNBUFFERED=1 venv/bin/python scripts/run_phase6_tpu.py --output {quote(remote+'/measurements')}", "run.log")
    finally:
