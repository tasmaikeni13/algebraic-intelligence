...
2. **Short-run hardware throughput:** The diagnostic recorded $\mathbf{2,915,698\text{ tok/s}}$ versus $\mathbf{3,231,782\text{ tok/s}}$, a ratio of $\mathbf{0.9022}$. The full 100,000-step rerun must independently satisfy the throughput gate.
...
6. **Empirical Scaling Roadmap (Phases 8 & 9):**
   - **Phase 8 (Systematic Hyperparameter Sweeping & Architecture Tuning):** Evaluates three preregistered candidates for each architecture across seeds 42, 43, and 44 on at least **600M tokens of FineWeb-Edu per run** (**18 runs total**, at least $10.8\text{B}$ tokens evaluated) on the Cloud TPU v4-32 Pod slice before freezing Phase 9 configurations.
